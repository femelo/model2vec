...
from model2vec.train.base import FinetunableStaticModel, FinetunableStaticEnsembleModel, EnsembleComponent, TextDataset, EnsembleTextDataset
...
class StaticEnsembleModelForClassification(FinetunableStaticEnsembleModel):
    def __init__(
        self,
        *,
        ...
        # multilabel flag will be set based on the type of `y` passed to fit.
        self.multilabel: bool = False
        super().__init__(components=components, out_dim=out_dim)
    ...
    def _prepare_dataset(self, X: list[str], y: LabelType, max_length: int = 512) -> TextDataset:
        ...
        for component in self.components:
            component_tokenized: list[list[int]] = [
                encoding.ids[:max_length] for encoding in component.tokenizer.encode_batch_fast(X, add_special_tokens=False)
            ]
            tokenized.append(component_tokenized)
        if self.multilabel:
            # Convert labels to multi-hot vectors
            num_classes = len(self.classes_)
            labels_tensor = torch.zeros(len(y), num_classes, dtype=torch.float)
            ...
            labels_tensor = torch.tensor([self.classes_.index(label) for label in cast(list[str], y)], dtype=torch.long)
        return EnsembleTextDataset(tokenized, labels_tensor)
    ...
    # TODO: implement StaticEnsembleModelPipeline
    # def to_pipeline(self) -> StaticEnsembleModelPipeline:
    #     """Convert the model to an sklearn pipeline."""
    #     static_models = self.to_static_models()

    #     random_state = np.random.RandomState(_RANDOM_SEED)
    #     n_items = len(self.classes)
    #     X = random_state.randn(n_items, sum([static_model.dim for static_model in static_models]))
    #     y = self.classes
    ...
    #     return StaticEnsembleModelPipeline(static_models, converted)
